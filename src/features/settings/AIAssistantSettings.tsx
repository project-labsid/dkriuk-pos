  ...
  return (
    <motion.div {...fadeIn} className="max-w-2xl space-y-6">
          ...
          return (
            <div key={f.label} className="flex items-center justify-between py-3 border-b last:border-b-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 flex items-center justify-center text-emerald-600 dark:text-emerald-400"><Icon className="w-4 h-4" /></div>
                <div><p className="text-sm font-medium">{f.label}</p><p className="text-[11px] text-muted-foreground">{f.desc}</p></div>
              </div>
              <button onClick={() => f.toggle(!f.state)} className="shrink-0">
                {f.state ? <ToggleRight className="w-10 h-10 text-emerald-600" /> : <ToggleLeft className="w-10 h-10 text-muted-foreground/50" />}
              </button>
            </div>
          );
        })}
      ...
      <div className="flex justify-end">
        <Button onClick={() => saveMut.mutate({ ai_smart_suggestions: String(smartSuggestions), ai_auto_categorize: String(autoCategorize), ai_sales_forecast: String(salesForecast), ai_restock_alert: String(restockAlert), ai_customer_insight: String(customerInsight), ai_model: aiModel })} disabled={saveMut.isPending} className="bg-emerald-600 hover:bg-emerald-700">
