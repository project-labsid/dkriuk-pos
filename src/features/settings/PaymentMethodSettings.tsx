...
interface PaymentMethod {
  id: string;
  name: string;
  type: 'cash' | 'qris' | 'transfer' | 'ewallet' | 'credit' | 'other';
  isActive: boolean;
  isDefault: boolean;
}
...
const typeColors: Record<PaymentMethod['type'], string> = {
  cash: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400',
  qris: 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400',
  transfer: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
  ewallet: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
...
export default function PaymentMethodSettings() {
  ...
  const handleSetDefault = (id: string) => {
    ...
    toast.success('Metode default berhasil diubah');
  };

  const handleDelete = (id: string) => {
    const method = methods.find((m) => m.id === id);
    if (method?.isDefault) {
      toast.error('Tidak bisa menghapus metode default');
      return;
    }
            ...
            {methods.map((method) => (
              <div
                ...
                className={`flex items-center gap-3 rounded-lg border p-4 transition-colors ${
                  !method.isActive ? 'opacity-50' : 'hover:bg-muted/50'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className={typeColors[method.type]}>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Masukkan nama dan jenis metode pembayaran baru
                  </DialogDescription>
                </DialogHeader>
